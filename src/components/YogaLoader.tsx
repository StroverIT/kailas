...
export default function YogaLoader({ show, label = "Preparing..." }: YogaLoaderProps) {
  ...
  const loopRefs = useRef<gsap.core.Tween[]>([]);
  ...
  useGSAP(() => {
    ...
    if (show) {
      ...
      // 2. Entrance (elastic pop)
      const enter = gsap.timeline({ defaults: { ease: "expo.out" } });
      enter
        .to(overlay, { autoAlpha: 1, duration: 0.6 })
        .to(
          sunContainer,
          {
            scale: 1,
        ...
        .to(glow, { scale: 1, opacity: 0.5, duration: 2 }, "-=1.2")
        .to(labelEl, { y: 0, autoAlpha: 1, duration: 0.8 }, "-=1.4");

      // 3. Loop animations
      const rotation = gsap.to(sunGraphic, {
        rotation: 360,
      ...
      const pulse = gsap.to(sunGraphic, {
        scale: 1.12,
        transformOrigin: "center center",
        duration: 1.8,
        repeat: -1,
      ...
      loopRefs.current = [rotation, pulse, glowPulse];
    } else {
      // 4. Exit animation
      loopRefs.current.forEach((t) => t.kill());
      loopRefs.current = [];
